...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--root-window", action="store_true", help="Capture the X11 root window instead of a matched window.")
    ...
    parser.add_argument("--crop-x", type=int, default=0, help="Optional crop x offset after capture.")
    parser.add_argument("--crop-y", type=int, default=0, help="Optional crop y offset after capture.")
    parser.add_argument("--crop-width", type=int, default=0, help="Optional crop width after capture. 0 keeps full width.")
    parser.add_argument("--crop-height", type=int, default=0, help="Optional crop height after capture. 0 keeps full height.")
...
def _capture_root_window() -> np.ndarray:
    completed = subprocess.run(
        ["xwd", "-silent", "-root"],
        check=True,
        capture_output=True,
...
def _crop_frame(frame: np.ndarray, crop_x: int, crop_y: int, crop_width: int, crop_height: int) -> np.ndarray:
    height, width = frame.shape[:2]
    x0 = max(0, crop_x)
    y0 = max(0, crop_y)
    x1 = width if crop_width <= 0 else min(width, x0 + crop_width)
    y1 = height if crop_height <= 0 else min(height, y0 + crop_height)
    if x0 >= x1 or y0 >= y1:
        raise ValueError(
            f"invalid crop rectangle x={crop_x} y={crop_y} width={crop_width} height={crop_height} for frame {width}x{height}"
        )
...
def main() -> int:
    ...
    if args.root_window:
        metadata["window_id"] = "root"
        metadata["window_title"] = "X11 Root Window"
    else:
    ...
    try:
        while True:
            ...
            try:
                if args.root_window:
                    frame = _capture_root_window()
                else:
                    frame = _capture_window(match.window_id)
                frame = _crop_frame(frame, args.crop_x, args.crop_y, args.crop_width, args.crop_height)
                consecutive_capture_failures = 0
