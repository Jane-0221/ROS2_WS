...
控制舵机角度示例：
- 第一个舵机转到45度: {"action": "servo", "id": 0, "angle": 0.785, "reply": "舵机0已转到45度"}
- 第二个舵机转到90度: {"action": "servo", "id": 1, "angle": 1.571, "reply": "舵机1已转到90度"}
- 第三个舵机转到-30度: {"action": "servo", "id": 2, "angle": -0.524, "reply": "舵机2已转到-30度"}
- 头部舵机归零: {"action": "servo", "id": 0, "angle": 0.0, "reply": "舵机0已归零"}
        ...
        try:
            ...
            if json_str:
                ...
                if action == 'chat':
                    ...
                    self.get_logger().info(f'舵机控制: ID={servo_id}, 角度={angle}弧度')
