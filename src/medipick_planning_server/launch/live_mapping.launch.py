...
def build_slam_actions(context):
    if LaunchConfiguration("start_slam").perform(context).strip().lower() != "true":
        return []
    ...
    if LaunchConfiguration("start_rtabmap_viz").perform(context).strip().lower() == "true":
        slam_actions.append(
            Node(
                package="rtabmap_viz",
    ...
    if LaunchConfiguration("wait_for_camera_tf_ready").perform(context).strip().lower() != "true":
        return slam_actions
