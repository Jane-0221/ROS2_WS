...
def cleanup_voice_scripts():
    ...
    print("\n🎯 推荐使用:")
    print("  实时语音识别: python3 real_time_speech_recognition_pyaudio.py")
    print("\n💡 下一步建议:")
...
'''
    ...
    script_path = os.path.join(os.getcwd(), 'funasr_robot_control.py')
    
    if not os.path.exists(script_path):
        with open(script_path, 'w', encoding='utf-8') as f:
            f.write(funasr_control_content)
        print(f"✅ 创建FunASR机器人控制脚本: funasr_robot_control.py")
    else:
        print(f"ℹ️  脚本已存在: funasr_robot_control.py")
